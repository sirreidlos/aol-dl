...
def visualize_sr(img, halve=False):
    ...
    if halve:
        hr_img = hr_img.resize(
            (int(hr_img.width / 2), int(hr_img.height / 2)), Image.Resampling.LANCZOS
        )
    lr_img = hr_img.resize(
        (int(hr_img.width / 4), int(hr_img.height / 4)), Image.Resampling.BICUBIC
    )

    # Bicubic Upsampling
    bicubic_img = lr_img.resize((hr_img.width, hr_img.height), Image.Resampling.BICUBIC)
    ...
    draw.text(
        xy=(
            margin + bicubic_img.width / 2.0 - width / 2.0,
            margin - height - 5.0,
        ),
        text="Bicubic",
        font=font,
    ...
    draw.text(
        xy=(
            2 * margin + bicubic_img.width + sr_img_srresnet.width / 2 - width / 2,
            margin - height - 5,
        ),
        text="SRResNet",
        font=font,
    ...
    draw.text(
        xy=(
            2 * margin + bicubic_img.width + sr_img_srresnet.width / 2 - width / 2,
            2 * margin + sr_img_srresnet.height - height - 1,
        ),
        text="Original HR",
        font=font,
        fill="black",
...
if __name__ == "__main__":
    grid_img = visualize_sr("./data/Set14/original/baboon.png")
